...
import { updateOpenInternship } from "../../services/applicationService";
import { toast } from "react-toastify";

export default function EditInternshipModal({ isOpen, onClose, internship, onSuccess }) {
  ...
  // Prefill form when internship prop is loaded
  useEffect(() => {
    if (internship) {
      setForm({
        ...
        deadline: formatDateForInput(internship.deadline),
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await updateOpenInternship(internship._id, form);
      if (res && res.success) {
        toast.success("Internship updated successfully!");
        onSuccess?.();
        onClose();
      } else {
        throw new Error(res?.message || "Failed to update internship");
      }
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data?.message || err.message || "Failed to update internship");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        >
          ...
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-indigo-50/50 to-blue-50/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-extrabold text-slate-800 leading-tight">Edit Internship</h2>
                <p className="text-xs font-semibold text-slate-400 mt-0.5">Modify open internship details</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black rounded-2xl disabled:opacity-50 transition-all shadow-md shadow-indigo-600/10 cursor-pointer"
              >
                {submitting ? (
                  <>
