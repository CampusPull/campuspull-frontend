...
export default function Explore() {
  const {
    ...
    activeRole,
    setActiveRole,
    isGuest,               // FIX: from exploreContext
    showAuthModal,         // FIX: from exploreContext
    setShowAuthModal,      // FIX: from exploreContext
  } = useExplore();
  ...
  return (
    <div className="min-h-screen p-4 md:p-10 bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <p className="text-center text-red-500 mt-6 bg-red-50 p-4 rounded-xl border border-red-100">
            {error}
          </p>
        )}
      ...
      {/* FIX: Signup modal for all restricted actions */}
      {showAuthModal && (
        <SignupModal
          isOpen={showAuthModal}
          onClose={() => setShowAuthModal(false)}
          message="Create an account to connect with people"
        />
      )}
    </div>
  );
...
// --- CARD COMPONENT ---
const UserCard = ({ cardUser, sendRequest, outgoingRequestIds, isGuest, onRestrictedAction }) => {
  const { acceptedConnectionIds, incomingRequests } = useExplore();
  const { user: loggedInUser } = useAuth();

  // FIX: Don't hide cards for guests (loggedInUser is null, so _id check would break)
  if (!isGuest && loggedInUser?._id === cardUser._id) return null;

  const isConnected = !isGuest && acceptedConnectionIds.has(cardUser._id);
  const isRequestSent = !isGuest && outgoingRequestIds.has(cardUser._id);
  const hasIncomingRequest = !isGuest && incomingRequests.some(req => req.requester._id === cardUser._id);

  const imgSrc = cardUser.profileImage || null;

  // FIX: Guest always sees "Connect" button that triggers modal
  ...
  return (
    <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 p-6 flex flex-col items-center group">
      ...
      <div className="relative">
        <Link to={isGuest ? "#" : `/profile/${cardUser._id}`} onClick={isGuest ? (e) => { e.preventDefault(); onRestrictedAction(); } : undefined}>
      ...
      {isGuest ? (
        <button
          ...
          className="hover:underline decoration-indigo-500 underline-offset-4 text-lg font-bold text-gray-800 hover:text-indigo-600 transition-colors text-center"
        >
          {cardUser.name}
        </button>
      ) : (
      ...
      {/* FIX: hide email/linkedin for guests — Sakshi excludes these from public API anyway */}
      {!isGuest && (
        <div className="flex items-center justify-center gap-4 mb-5 w-full border-t border-gray-100 pt-4">
          {cardUser.email && (
            <a href={`mailto:${cardUser.email}`} className="text-gray-400 hover:text-indigo-600 text-xl transition-colors">
              <FaEnvelope />
            </a>
          )}
          {cardUser.linkedin && (
            <a href={cardUser.linkedin} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-blue-600 text-xl transition-colors">
              <FaLinkedin />
            </a>
          )}
        </div>
