...
const LoadingSkeleton = ({ viewMode = 'grid', count = 6 }) => {
  const SkeletonCard = ({ isListView = false }) => (
    <div className={`bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm ${isListView ? 'p-6' : ''}`}>
      {!isListView && (
      ...
      <div className={isListView ? 'flex items-start space-x-4' : 'p-4'}>
        {isListView && (
          <div className="flex-shrink-0 w-20 h-20 bg-slate-100 rounded-2xl animate-pulse"></div>
        )}
        
        <div className="flex-1 space-y-3">
          <div className="flex items-center justify-between">
            <div className="h-4 bg-slate-100 rounded animate-pulse w-24"></div>
            {!isListView && <div className="h-8 w-8 bg-slate-100 rounded-full animate-pulse"></div>}
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100">
            ...
            <div className="flex space-x-2">
              <div className="h-8 bg-slate-100 rounded animate-pulse w-16"></div>
              <div className="h-8 bg-slate-100 rounded animate-pulse w-20"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 space-y-4">
        ...
        {[1, 2, 3]?.map((i) => (
          <div key={i} className="flex items-start space-x-4">
            <div className="w-8 h-8 bg-slate-100 rounded-full animate-pulse flex-shrink-0"></div>
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div className="space-y-2">
            <div className="h-5 bg-slate-100 rounded animate-pulse w-24"></div>
            <div className="h-4 bg-slate-100 rounded animate-pulse w-20"></div>
          </div>
        </div>
        <div className="space-y-2">
          <div className="h-6 bg-slate-100 rounded-full animate-pulse w-16"></div>
          <div className="h-6 bg-slate-100 rounded-full animate-pulse w-12"></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        {[1, 2, 3]?.map((i) => (
          <div key={i} className="text-center space-y-1">
            <div className="h-6 bg-slate-100 rounded animate-pulse w-8 mx-auto"></div>
            <div className="h-3 bg-slate-100 rounded animate-pulse w-12 mx-auto"></div>
          </div>
        ))}
      </div>
